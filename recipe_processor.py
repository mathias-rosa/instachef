...
class RecipeProcessor:
    ...
    def process_reel(self, reel_url: str) -> Recipe | None:
        download_result = self.instagram.download_reel(reel_url)
        if not download_result:
            return None

        video_path, caption, shortcode = download_result
        ...
        recipe = self._parse_recipe(recipe_json)
    ...
    @staticmethod
    def _parse_recipe(recipe_json: str) -> Recipe | None:
        try:
